...
| `submitGoldenGameDirection(direction: 'L' \| 'R')` | Sends the reaction-key reply for the golden fish minigame. The library never decides the direction itself — the consumer must listen to `GOLDEN_FISHING_STATUS` and call this once it picks `'L'` or `'R'`. No-op before `connect()` or before the room is confirmed entered. |
...
| `GOLDEN_FISHING_STARTED` | `fishId: number, seconds: number`                                                           | A golden fish minigame started. Not sent on every cast — only when a golden fish bites, see the header note below. The server doesn't always send this before the first `GOLDEN_FISHING_STATUS`, so treat either event as "minigame is now active" |
| `GOLDEN_FISHING_STATUS`  | `balance: number, barProgress: number, seconds: number`                                     | Golden fish minigame tick. `balance` is a needle drifting toward one side — react by calling `submitGoldenGameDirection('L' \| 'R')` once it crosses your chosen threshold; the library does not decide this for you                               |
| `GOLDEN_FISHING_ENDED`   | —                                                                                           | The golden fish minigame ended (caught or missed) — the outcome itself arrives separately via the following `FISH_CAUGHT_MSG`                                                                                                                      |
...
| 1108   | `GOLDEN_STATUS`         | Golden fish minigame tick. Body: `balance:int, barProgress:int, seconds:int` — `balance` is a needle that drifts toward one side. `HabboPlayerManager` only forwards these fields via `GOLDEN_FISHING_STATUS`; it does not decide or send anything itself. Reply with outgoing `FHM`(1101) + `"L"`/`"R"` via `Bot.submitGoldenGameDirection()` once `balance` crosses your chosen threshold, to keep the fish hooked |
...
| 2      | `ROOM_DIRECTORY`     |                                                                                                                                                                     |
| 4      | `LOGIN`              |                                                                                                                                                                     |
| 5      | `VERSIONCHECK`       | confirmed in the decompiled client (`hh_entry`, `Login Handler Class.startSession`): `[buildNumber:int, clientUrl:str, extVarsUrl:str]`. `buildNumber` is the internal build id, offset from the public version by a constant `+803` (e.g. public 330 → 1133) — a convention, not a runtime formula. `clientUrl` is the literal string `"2"` in practice (the real source only sends the true client URL when embedded as a browser plugin with a matching `src` param, never the case here). `extVarsUrl` is the real gamedata URL |
| 6      | `UNIQUEID`           | confirmed in the same source: `[#string: getMachineID()]`. Despite the old name, this is a machine/device fingerprint (native Xtra `Device_GetMachineId()`, falling back to a timestamp-based id), not a session/auth token — the server only validates the format (`BX1-XXXX-XXXX-XXXX-XXXX-XXXX`), not the algorithm |
...
| 1101   | `FHM`                | reaction-key reply for the golden fish minigame. Body: a single string, `"L"` or `"R"`; sent via `Bot.submitGoldenGameDirection()`                                  |
| 181    | `GET_SESSION_PARAMETERS` | confirmed in the decompiled client; sent right after `VERSIONCHECK`/`UNIQUEID` as part of the post-crypto burst, no body                                       |
| 196    | `PONG`               |                                                                                                                                                                     |
| 202    | `GENERATE_KEY`       |                                                                                                                                                                     |
| 206    | `CLIENT_INIT`        |                                                                                                                                                                     |
...
- [x] Golden fish minigame — `HabboPlayerManager` forwards `GOLDEN_FISHING_STARTED`/`STATUS`/`ENDED` and exposes `Bot.submitGoldenGameDirection('L' | 'R')`; the library deliberately does not decide the direction itself, that's on the consumer (see `habfish-cli` for a reference implementation: ±3 threshold, ~200ms debounce)
